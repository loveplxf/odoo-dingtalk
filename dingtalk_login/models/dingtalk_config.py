...
class DingTalkConfig(models.Model):
    ...
    def _constraint_app_key(self):
        ...
        """
        for res in self:
            oauth_id = self.env.ref('dingtalk_login.dingtalk_login_auth_oauth').sudo()
            oauth_id.write({
    ...
    def sync_employee_login_information(self):
        ...
        """
        self.ensure_one()
        # domain = [('company_id', '=', self.company_id.id), ('din_unionid', '!=', False), ('user_id', '!=', False)]
        domain = [('din_unionid', '!=', False), ('user_id', '!=', False)]
        for emp in self.env['hr.employee'].sudo().search(domain):
            emp.user_id.sudo().write({
                'oauth_uid': emp.din_unionid,
