...
class SynchronousEmployee(models.TransientModel):
        ...
        for company_id in self.company_ids.ids:
            ...
            for department in departments:
                ...
                while True:
                    ...
                    user = self.env['res.users'].with_user(SUPERUSER_ID).search([('id', '=', self.env.user.id)])
                    result_state = self._get_dingtalk_employee_list(client, department, offset, size,
                                                                    company, self.repeat_type, user)
                    if result_state:
                        offset = offset + size
                    else:
                        break
    ...
    def _get_dingtalk_employee_list(self, client, department, offset, size, company, repeat_type, user_id):
        ...
        try:
            ...
            _logger.info(">>>获取了%s员工", len(result.get('userlist')))
            ...
            raise exceptions.ValidationError("同步员工时发生异常：{}".format(str(e)))
