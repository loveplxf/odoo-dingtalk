...
class DingTalkLogin(OAuthLogin):
    ...
    def list_providers(self):
        ...
        """
        providers = super(DingTalkLogin, self).list_providers()
...
class DingtalkOAuthController(OAuthController):
        ...
        with registry.cursor() as cr:
            try:
                ...
                if app_key and auth_code:         # 针对钉钉登录的特殊处理
                    ...
                    dingtalk_config = env['dingtalk.config'].search([('app_key', '=', app_key)], limit=1)
                    ...
                    db, login, key = env['res.users'].sudo().auth_oauth('dingtalk_login', user_info.get('unionId'))
                else:
                    db, login, key = env['res.users'].sudo().auth_oauth('dingtalk_login', kw)
                cr.commit()
    ...
    def dingtalk_signin(self, **kw):
        ...
        return request.render('dingtalk_login.dingtalk_signin', {'corp_id': kw.get('corpId')})
    ...
    def dingtalk_signin_action(self, **kw):
        ...
        config = request.env['dingtalk.config'].sudo().search([('corp_id', '=', corp_id)], limit=1)
        ...
        with registry.cursor() as cr:
            try:
                env = api.Environment(cr, SUPERUSER_ID, {})
                db, login, key = env['res.users'].sudo().auth_oauth('dingtalk_login', employee.din_unionid)
                cr.commit()
                pre_uid = request.session.authenticate(db, login, key)
                resp = request.redirect(_get_login_redirect_url(pre_uid, '/web'), 303)
                resp.autocorrect_location_header = False
