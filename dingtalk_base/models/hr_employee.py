...
class HrEmployee(models.Model):
    ...
    def create_employee_user_by_form(self):
        _logger.info("自动创建用户")
        self.ensure_one()
        if self.user_id:
            _logger.info("用户已存在：{}".format(self.user_id))
            return
        domain = ['|', ('login', '=', self.work_email), ('login', '=', self.mobile_phone)]
        ...
        if user and not user.employee_id:
            ...
            values = {
                ...
                'oauth_uid': self.din_unionid,
                'oauth_access_token': self.din_unionid,
                'employee': True,
                'employee_id': self.id,
                'employee_ids': [(6, 0, [self.id])],
            }
            name_count = self.env['res.users'].with_user(SUPERUSER_ID).search_count([('name', 'like', self.name)])
            if name_count > 0:
                user_name = self.name + str(name_count + 1)
                values['name'] = user_name
            try:
                user = self.env['res.users'].with_user(SUPERUSER_ID).create(values)
            except Exception as e:
                _logger.info("创建用户失败：{}".format(e))
            self.write({'user_id': user.id})
        return {
        ...
        for employee in employee_list:
            ...
            if user and not user.employee_id:
                ...
                values = {
                    ...
                    'oauth_uid': employee.din_unionid,
                    'oauth_access_token': employee.din_unionid,
                    'employee': True,
